...
unsigned short Skylander::getShort(int block, int offset) {
    return data[block * 16 + offset] + data[block * 16 + offset + 1] * 0x100;
}
...
unsigned int Skylander::getHeroicChallenges() {
    ...
    return getByte(block, 0xc) + getByte(block, 0xd) * 0x100 + getByte(block, 0xe) * 0x10000 +
           getByte(block, 0xf) * 0x1000000;
}
