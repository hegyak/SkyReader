        ...
        case LIBUSB_TRANSFER_TYPE_ISOCHRONOUS:
            switch (sync_type) {
                case LIBUSB_ISO_SYNC_TYPE_NONE:
                    return "isochronous_none";
                case LIBUSB_ISO_SYNC_TYPE_ASYNC:
                    return "isochronous_asynchronous";
                case LIBUSB_ISO_SYNC_TYPE_ADAPTIVE:
                    return "isochronous_adaptive";
                case LIBUSB_ISO_SYNC_TYPE_SYNC:
                    return "isochronous_synchronous";
            }
            return "isochronous";
    ...
    for (i = 0; i < endpoints_no; i++) {
        ...
        if (epdesc->bEndpointAddress & 0x80)
            printf("found an IN End Point %d with attributes %s and address 0x%x\n", i,
                   endpoint_attribute(epdesc->bmAttributes), epdesc->bEndpointAddress & 0x7f);
        else
            printf("found an OUT End Point %d with attributes %s and address 0x%x\n", i,
                   endpoint_attribute(epdesc->bmAttributes), epdesc->bEndpointAddress);
    }
    libusb_free_config_descriptor(config);
