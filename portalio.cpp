...
DEFINE_GUID(xbox_guid,
0xB323AD0E, 0xADA8, 0x4d64,
...
BOOL OpenPortalHandleFromGUID(GUID guid, PHANDLE phPortalHandle) {
    ...
    *phPortalHandle = NULL;
    ...
    for (memberIndex = 0;
        ...
        if (!SetupDiGetDeviceInterfaceDetail(hDevInfo, &deviceInterfaceData, pInterfaceDetailData, requiredLength,
                                             &requiredLength, NULL)) {
            continue;
        }
        ...
        *phPortalHandle = CreateFile(
        ...
        if (*phPortalHandle != INVALID_HANDLE_VALUE) {
            if (HidD_GetAttributes(*phPortalHandle, &attributes)) {
                if (((attributes.VendorID == 0x12ba) || (attributes.VendorID == 0x54c)) ||
                    (attributes.VendorID == 0x1430)) {
...
void WriteSkylanderToPortal(unsigned char *encrypted_new_data, unsigned char *encrypted_old_data) {
    WriteSkylanderFile("D:\\Skylanders\\Editor\\Debug\\dspyro_u.spy",
                       encrypted_new_data);  // emulate portal for testing
}
